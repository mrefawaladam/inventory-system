...
<x-layout.page-header
    title="Transaksi Inbound"
    :breadcrumb-title="'Transaksi Inbound'"
/>
    ...
    <div class="card">
        <div class="card-body">
            <div class="d-flex justify-content-between align-items-center mb-3">
                <h5 class="card-title mb-0">Daftar Transaksi Inbound</h5>
                <a href="{{ route('inbound.create') }}" class="btn btn-primary">
                    <i class="ti ti-plus"></i> Transaksi Baru
                </a>
            </div>
            <div class="table-responsive">
                <table id="inbound-table" class="table table-striped table-bordered align-middle" style="width: 100%;">
                    <thead>
                        <tr>
...
$(document).ready(function() {
    if ($.fn.DataTable.isDataTable('#inbound-table')) {
        $('#inbound-table').DataTable().destroy();
    }

    const inboundTable = $('#inbound-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: {
            ...
            dataSrc: function(json) {
                $('#inbound-table tbody').find('.table-loading-overlay').remove();
                return json.data;
            }
        },
        columns: [
    ...
    inboundTable.on('processing.dt', function(e, settings, processing) {
        if (processing) {
                    ...
                    <tr class="table-loading-overlay-row">
                        <td colspan="9" style="position: relative; height: 300px; padding: 0;">
                            <div class="table-loading-overlay">
                                ...
                                <div class="loading-text">Memuat data transaksi inbound...</div>
                            </div>
                        </td>
                    </tr>
                `);
            ...
            $('#inbound-table tbody').find('.table-loading-overlay-row').remove();
        }
    });
});
</script>
